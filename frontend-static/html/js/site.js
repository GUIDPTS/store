...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.store('cart', {
    items: (() => { try { return JSON.parse(localStorage.getItem('cart_v1') || '[]'); } catch(e) { return []; } })(),
    ...
    save() { try { localStorage.setItem('cart_v1', JSON.stringify(this.items)); } catch(e) {} },
  ...
  Alpine.store('wishlist', {
    items: (() => { try { return JSON.parse(localStorage.getItem('wishlist_v1') || '[]'); } catch(e) { return []; } })(),

    add(product) {
      if (!this.has(product.id)) {
        this.items.push({ id: product.id, name: product.name, price: product.price, image: product.image || '', stock: product.stock_count || 0 });
        this.save();
      }
    },
    ...
    save() { try { localStorage.setItem('wishlist_v1', JSON.stringify(this.items)); } catch(e) {} },
  });
  ...
  Alpine.store('app', {
    ...
    async init() {
      const [s, c, u] = await Promise.allSettled([
        ...
        fetch('/api/categories/with-products').then(r => r.ok ? r.json() : fetch('/api/categories').then(r2 => r2.json())).catch(() => []),
        fetch('/api/user/info').then(r => r.ok ? r.json() : null).catch(() => null),
      ]);
