<template>
  <section class="cart py-80">
    <div class="container container-lg">
      <h4 class="fw-semibold mb-32">购物车</h4>

      <div v-if="!cart.items.length" class="text-center py-60">
        <i class="ph ph-shopping-cart d-block mb-16 text-gray-200" style="font-size: 4rem"></i>
        <p class="text-gray-400 mb-24">购物车为空</p>
        <NuxtLink to="/" class="btn btn-main rounded-pill px-32 py-12">去逛逛</NuxtLink>
      </div>

      <div v-else class="row gy-4">
        <div class="col-lg-8">
          <div class="border border-gray-100 rounded-16 overflow-hidden">
            <div
              v-for="item in cart.items"
              :key="item.id"
              class="d-flex align-items-center gap-16 p-20 border-bottom border-gray-100"
            >
              <NuxtLink :to="`/product/${item.id}`" class="flex-shrink-0">
                <img
                  :src="item.image || ''"
                  :alt="item.name"
                  style="
                    width: 72px;
                    height: 72px;
                    object-fit: contain;
                    border-radius: 8px;
                    background: #f9f9f9;
                  "
                />
              </NuxtLink>
              <div class="flex-1 min-w-0">
                <h6 class="fw-semibold mb-4 text-line-2">
                  <NuxtLink :to="`/product/${item.id}`" class="text-heading hover-text-main-600">{{
                    item.name
                  }}</NuxtLink>
                </h6>
                <p class="text-main-600 fw-bold mb-0">¥{{ Number(item.price).toFixed(2) }}</p>
              </div>
              <div class="d-flex align-items-center gap-8 flex-shrink-0">
                <div
                  class="border border-gray-100 rounded-pill py-6 px-12 d-flex align-items-center gap-4"
                >
                  <button
                    type="button"
                    class="btn-reset text-gray-700"
                    @click="cart.updateQty(item.id, item.qty - 1)"
                  >
                    <i class="ph ph-minus text-sm"></i>
                  </button>
                  <span class="text-sm fw-medium" style="min-width: 24px; text-align: center">{{
                    item.qty
                  }}</span>
                  <button
                    type="button"
                    class="btn-reset text-gray-700"
                    @click="cart.updateQty(item.id, item.qty + 1)"
                  >
                    <i class="ph ph-plus text-sm"></i>
                  </button>
                </div>
                <button
                  type="button"
                  class="rounded-circle bg-danger-50 text-danger-600 d-flex align-items-center justify-content-center"
                  style="width: 32px; height: 32px; border: 0; cursor: pointer"
                  @click="cart.remove(item.id)"
                >
                  <i class="ph ph-trash"></i>
                </button>
              </div>
            </div>
          </div>
        </div>

        <div class="col-lg-4">
          <div class="border border-gray-100 rounded-16 p-24">
            <h6 class="fw-semibold mb-20">订单摘要</h6>
            <div class="d-flex justify-content-between mb-12">
              <span class="text-sm text-gray-600">商品总数</span>
              <span class="text-sm fw-medium">{{ cart.count }} 件</span>
            </div>
            <div class="d-flex justify-content-between mb-24 border-top border-gray-100 pt-12">
              <span class="fw-semibold">合计</span>
              <span class="fw-bold text-main-600 text-xl">¥{{ cart.total.toFixed(2) }}</span>
            </div>
            <NuxtLink
              v-if="auth.isAuthenticated"
              to="/checkout"
              class="btn btn-main w-100 py-14 rounded-pill fw-semibold"
              >前往结算</NuxtLink
            >
            <a
              v-else
              href="/auth/login?redirect=/checkout"
              class="btn btn-main w-100 py-14 rounded-pill fw-semibold"
              >登录后结算</a
            >
            <NuxtLink
              to="/"
              class="btn btn-white w-100 py-14 rounded-pill fw-semibold mt-12 border border-gray-200"
              >继续购物</NuxtLink
            >
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
import { useCartStore } from "~/stores/cart";
import { useAuthStore } from "~/stores/auth";

const cart = useCartStore();
const auth = useAuthStore();
</script>
